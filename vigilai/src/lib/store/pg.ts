 ...
 *
 * Activa este backend definiendo DATABASE_URL. Las migraciones de db/migrations
 * se aplican automáticamente en el primer uso (y también con `npm run migrate`).
...
export class PgRepo implements Repo {
  ...
  /** Aplica las migraciones pendientes una sola vez. */
  private init(): Promise<void> {
    if (!this.ready) {
      this.ready = runMigrations(this.pool).then(
        () => undefined,
        (err) => {
